...
<head>
    ...
    <title>Modifier {{ $equipment->name }} - Equipment Tracker</title>
...
<body class="app-container">
    ...
    <header class="app-header">
        ...
        <div class="equipment-detail-title">
            <h1 class="app-title">Modifier équipement</h1>
        </div>
                ...
                <form method="POST" action="{{ route('admin.equipment.update', $equipment) }}" enctype="multipart/form-data" class="space-y-6">
                    ...
                    <div>
                        ...
                        <select name="status" id="status" required
                            ...
                            <option value="rented" {{ old('status', $equipment->status) === 'rented' ? 'selected' : '' }}>En cours d'utilisation</option>
                        ...
                        <select name="current_location_id" id="current_location_id" required
                            ...
                            @foreach($locations as $location)
                                <option value="{{ $location->id }}" 
                                        {{ old('current_location_id', $equipment->current_location_id) == $location->id ? 'selected' : '' }}>
                                    {{ $location->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <textarea name="notes" id="notes" rows="3"
                                  class="block w-full px-3 py-3 border border-gray-300 rounded-lg focus:ring-1 focus:ring-blue-500 focus:border-blue-500"
                                  placeholder="Remarques, état de l'équipement, instructions spéciales...">{{ old('notes', $equipment->notes) }}</textarea>
                        @error('notes')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
