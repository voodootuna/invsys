...
<head>
    ...
    <title>Ajouter un lieu - Equipment Tracker</title>
...
<body class="app-container">
    ...
    <header class="app-header">
        ...
        <div class="equipment-detail-title">
            <h1 class="app-title">Ajouter lieu</h1>
        </div>
                ...
                <form method="POST" action="{{ route('admin.locations.store') }}" class="space-y-6">
                    ...
                    <div>
                        ...
                        <select name="type" id="type" required
                            ...
                            <option value="client" {{ old('type') === 'client' ? 'selected' : '' }}>Chez client</option>
                            <option value="job_site" {{ old('type') === 'job_site' ? 'selected' : '' }}>Chantier</option>
                        </select>
                        @error('type')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    ...
                    <div>
                        ...
                        <textarea name="address" id="address" rows="3"
                                  class="block w-full px-3 py-3 border border-gray-300 rounded-lg focus:ring-1 focus:ring-blue-500 focus:border-blue-500"
                                  placeholder="Adresse complète du lieu...">{{ old('address') }}</textarea>
                        @error('address')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
